...
/**
 * Prueba de integración HTTP para el controlador /register.
 * Levanta la app Express completa y verifica respuestas reales.
 */
describe('RegistryController (IT – HTTP)', () => {
  ...
  it('shouldReturnDeadForDeceasedPerson', async () => {
    const res = await request(app)
      .post('/register')
      .set('Content-Type', 'application/json')
      .send({ name: 'Difunto', id: 300, age: 40, gender: 'MALE', alive: false });
  ...
  it('shouldReturnDuplicatedForSameId', async () => {
    const app2 = createApp(); // instancia fresca para aislar estado
    await request(app2)
      .post('/register')
      .send({ name: 'Primero', id: 400, age: 25, gender: 'FEMALE', alive: true });

    const res = await request(app2)
      .post('/register')
      .set('Content-Type', 'application/json')
      .send({ name: 'Segundo', id: 400, age: 25, gender: 'FEMALE', alive: true });
...
/**
 * Prueba de integración HTTP para el endpoint de votantes /api/v1/voters/register.
 */
